...
def _build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Convert a LTspice netlist plus symbol-pose JSON into routed wiring JSON.",
    )
    parser.add_argument(
        "netlist_filepath",
    ...
    parser.add_argument(
        ...
        help="Path to the symbol-pose JSON file.",
    )
    parser.add_argument(
        "--out",
        default=None,
        help="Optional output wire JSON path. Defaults to the netlist stem plus '_wires.json'.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    if not netlist_input.is_file():
        ...
        return 1
    symbol_pose_input = Path(arguments.symbol_pose_filepath)
    if not symbol_pose_input.is_file():
        print(f"{symbol_pose_input}: not a file", file=sys.stderr)
        return 1
    output_path = (
        arguments.out
        if arguments.out is not None
        else str(Path(arguments.netlist_filepath).with_suffix("").with_suffix("") + "_wires.json")
    )
    result = ltspice_netlist_to_wiring(
        str(netlist_input),
        str(symbol_pose_input),
        output_path,
        convert_settings,
    )
    if not result[0]:
