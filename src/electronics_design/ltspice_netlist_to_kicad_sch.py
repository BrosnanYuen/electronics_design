...
from .kicad_symbol import is_valid_kicad_symbol_file  # Validate dynamically generated symbol files.
...
_KICAD_LIB_ID_HINT_PATTERN = re.compile(r"^\*\s*KICAD_SYMBOL_LIB_ID\s+(\S+)\s+(\S+)\s*$", re.IGNORECASE)  # Match the original KiCad library identifier recorded by the forward converter.

_KICAD_SIM_PINS_PATTERN = re.compile(r"^\*\s*KICAD_SIM_PINS\s+(\S+)\s+(.+?)\s*$", re.IGNORECASE)  # Match the authored Sim.Pins role mapping recorded by the forward converter.

_KICAD_VALUE_PATTERN = re.compile(r"^\*\s*KICAD_VALUE\s+(\S+)\s+(.+?)\s*$", re.IGNORECASE)  # Match the authored display value recorded by the forward converter.
...
_KICAD_SCH_PAGE_A3_WIDTH = 420.0  # A3 landscape page width in mm.
_KICAD_SCH_PAGE_A3_HEIGHT = 297.0  # A3 landscape page height in mm.
_KICAD_SCH_A3_SYMBOL_THRESHOLD = 80  # Designs with more symbols than this default to A3 paper.
...
def _build_schematic_text(lines: Sequence[str], input_path: str, settings: Dict[str, Any]) -> BuildResult:  # Build the full KiCad schematic text from netlist lines.
    ...
    try:  # Run the conversion stages inside the scratch directory lifetime.
        ...
        lib_id_hints = _parse_lib_id_hints(lines) if generated_by_converter else {}  # Restore exact KiCad symbols only for converter-produced decks.
        sim_pins_hints = _parse_sim_pins_hints(lines) if generated_by_converter else {}  # Restore authored role mappings only for converter-produced decks.
        value_hints = _parse_value_hints(lines) if generated_by_converter else {}  # Restore authored display values only for converter-produced decks.
        library_names = _collect_library_file_names(lines)  # Read the referenced library filenames for instance Sim.Library recovery.
        subcircuit_definitions = _collect_subcircuit_definitions(lines, settings)  # Read subcircuit port lists for dynamic symbol generation.
        records_result = _build_component_records(elements, model_types, settings, temp_directory, generated_by_converter, lib_id_hints, sim_pins_hints, value_hints, library_names, subcircuit_definitions)  # Resolve symbols and build component records.
        ...
        hinted_lib_ids = sorted({str(record["lib_id"]) for record in records if record.get("hinted")})  # Collect hinted library identifiers for annotation preservation.
        text = _assemble_schematic(input_path, settings, embedded_symbols, schematic_nodes, simulation_text_nodes, hinted_lib_ids, symbol_count=len(records))  # Assemble the final schematic text.
...
_SUBCKT_HEADER_PATTERN = re.compile(r"^\.subckt\s+(\S+)\s*(.*)$", re.IGNORECASE)  # Detect subcircuit header lines and their port lists.
...
def _parse_lib_id_hints(lines: Sequence[str]) -> Dict[str, str]:  # Read the original KiCad library identifiers recorded by the forward converter.
    hints: Dict[str, str] = {}  # Collect hints keyed by the emitted device instance name.
    for raw_line in lines:  # Walk every netlist line.
        match = _KICAD_LIB_ID_HINT_PATTERN.fullmatch(raw_line.strip())  # Match the recorded hint comment.
        if match is not None:  # Keep only complete hint records.
            hints[match.group(1).upper()] = match.group(2)  # Store the lib_id under the uppercase instance name.
    return hints  # Return the collected library identifier hints.


def _parse_sim_pins_hints(lines: Sequence[str]) -> Dict[str, str]:  # Read the authored Sim.Pins role mappings recorded by the forward converter.
    hints: Dict[str, str] = {}  # Collect hints keyed by the emitted device instance name.
    for raw_line in lines:  # Walk every netlist line.
        match = _KICAD_SIM_PINS_PATTERN.fullmatch(raw_line.strip())  # Match the recorded role mapping comment.
        if match is not None:  # Keep only complete role records.
            hints[match.group(1).upper()] = match.group(2).strip()  # Store the role tokens under the uppercase instance name.
    return hints  # Return the collected role mappings.


def _parse_value_hints(lines: Sequence[str]) -> Dict[str, str]:  # Read the authored display values recorded by the forward converter.
    hints: Dict[str, str] = {}  # Collect hints keyed by the emitted device instance name.
    for raw_line in lines:  # Walk every netlist line.
        match = _KICAD_VALUE_PATTERN.fullmatch(raw_line.strip())  # Match the recorded value comment.
        if match is not None:  # Keep only complete value records.
            hints[match.group(1).upper()] = match.group(2).strip()  # Store the value text under the uppercase instance name.
    return hints  # Return the collected display values.


def _collect_library_file_names(lines: Sequence[str]) -> List[str]:  # Read the filenames referenced by include and library directives.
    library_names: List[str] = []  # Collect the referenced library filenames in first-use order.
    for raw_line in lines:  # Walk every netlist line.
    ...
    return library_names  # Return the collected library filenames.


def _collect_subcircuit_definitions(lines: Sequence[str], settings: Dict[str, Any]) -> Dict[str, List[str]]:  # Collect subcircuit port lists from the deck and its referenced libraries.
    definitions: Dict[str, List[str]] = {}  # Collect port lists keyed by subcircuit name.
    candidate_lines: List[str] = list(lines)  # Start with the inline deck lines.
    search_roots = _ltspice_search_roots(settings)  # Collect the configured search roots once.
    for library_name in _collect_library_file_names(lines):  # Walk every referenced library.
        ...
        if not read_result[0]:  # Skip unreadable libraries.
            continue  # Move to the next reference.
        candidate_lines.extend(read_result[1].splitlines())  # Append the library lines for header scanning.
    for raw_line in candidate_lines:  # Walk every candidate line.
        stripped_line = raw_line.strip()  # Normalize leading whitespace.
        match = _SUBCKT_HEADER_PATTERN.match(stripped_line)  # Match subcircuit header lines.
        if match is None:  # Skip non-header lines.
            continue  # Move to the next line.
        subcircuit_name = match.group(1)  # Read the subcircuit name.
        port_names = [token for token in match.group(2).split() if "=" not in token]  # Keep bare port tokens and drop parameter assignments.
        definitions[subcircuit_name] = port_names  # Record the port list under the subcircuit name.
    return definitions  # Return the collected subcircuit definitions.
...
def _candidate_lib_ids(element: ParsedElement, model_types: Dict[str, str], generated_by_converter: bool = False, hinted_lib_id: str = "") -> List[str]:  # Derive candidate KiCad library identifiers for one device.
    ...
    if prefix == "X":  # Subcircuit calls prefer the recorded KiCad symbol before the bare subcircuit name.
        if hinted_lib_id:  # The forward converter recorded the exact authored symbol.
            candidates = [hinted_lib_id]  # Restore the original library identifier first.
            if payload_text and payload_text not in candidates:  # Keep the bare-name fallback when the hint fails to resolve.
                candidates.append(payload_text)  # Preserve the subcircuit-name candidate.
            return candidates  # Return the hint-led candidates.
...
def _resolve_symbol(  # Resolve one device to a KiCad symbol from kicad_path or the LTspice ASY fallback.
    ...
    lib_ids: Sequence[str],  # Accept the ordered candidate library identifiers.
    ...
    allow_generic_subcircuits: bool = False,  # Accept generated symbols only for converter-produced decks with complete pin metadata.
    ...
    for lib_id in lib_ids:  # Walk the candidate library identifiers in order.
        ...
        if pins_result[0] and _subcircuit_round_trips(element, symbol_node):  # Require usable pin graphics and a round-trippable subcircuit identity.
            pin_count = len(pins_result[1])  # Read the symbol pin count.
    ...
    if element.prefix == "X" and allow_generic_subcircuits:  # Dynamically generate a symbol only when the trusted generator marker proves the deck's schematic provenance.
        ...
        port_names: List[str] = []  # Collect the pin names for the generated symbol.
        if subcircuit_definitions is not None:  # Prefer the subcircuit port list from the deck or its libraries.
            port_names = list(subcircuit_definitions.get(subcircuit_name, ()))  # Use the authored model ports.
        if not port_names and element.nodes:  # Use the netlist node names as informative pin labels.
            port_names = list(element.nodes)  # Name pins after their connected nets.
        if not port_names and hint_roles:  # Fall back to the recorded Sim.Pins roles when no definition is available.
            port_names = [token.split("=", 1)[1].strip() if "=" in token else token for token in hint_roles.split()]  # Derive port names from the role mapping.
...
def _extra_no_connect_pins_match(element: ParsedElement, pin_count: int, node_count: int, symbol_node: SExp) -> bool:  # Decide whether a hinted symbol's unlisted pins are all no-connect ports.
    if pin_count < node_count:  # Symbols with fewer pins than netlist nodes cannot represent the device.
        return False  # Return False for undersized symbols.
    excess = pin_count - node_count  # Count the symbol pins that the deck does not list.
...
def _no_connect_pin_numbers(symbol_node: SExp) -> Set[str]:  # Collect the pin numbers whose electrical type is no_connect.
    numbers: Set[str] = set()  # Collect the no-connect pin numbers.
    for sub_symbol in symbol_node.find_children("symbol"):  # Walk the nested unit sub-symbols.
        for pin_node in sub_symbol.find_children("pin"):  # Walk the pin definitions.
            atoms = [child.value for child in pin_node.children if child.is_atom]  # Collect the pin type atoms.
            if not atoms or str(atoms[0]).lower() != "no_connect":  # Keep only no-connect typed pins.
                continue  # Move to the next pin.
            number_node = pin_node.find_child("number")  # Locate the pin number section.
            if number_node is None:  # Skip pins without a number section.
                continue  # Move to the next pin.
            number_values = [child.value for child in number_node.children if child.is_atom]  # Collect the number atoms.
            if number_values:  # Keep pins with a readable number.
                numbers.add(str(number_values[0]))  # Record the no-connect pin number.
    return numbers  # Return the collected no-connect pin numbers.
...
def _build_component_records(  # Resolve symbols and build one component record per device element.
    ...
    lib_id_hints: Optional[Dict[str, str]] = None,  # Accept forward-converter library identifier hints keyed by instance name.
    sim_pins_hints: Optional[Dict[str, str]] = None,  # Accept forward-converter Sim.Pins role hints keyed by instance name.
    value_hints: Optional[Dict[str, str]] = None,  # Accept forward-converter display value hints keyed by instance name.
    library_names: Sequence[str] = (),  # Accept the deck's referenced library filenames for Sim.Library recovery.
    subcircuit_definitions: Optional[Mapping[str, Sequence[str]]] = None,  # Accept subcircuit port lists for dynamic symbol generation.
    ...
    for element in elements:  # Walk every parsed device element.
        ...
        instance_key = str(element.tokens[0]).upper()  # Normalize the instance name for hint lookup.
        hinted_lib_id = lib_id_hints.get(instance_key, "") if lib_id_hints else ""  # Read the forward-converter library identifier hint.
        hint_roles = sim_pins_hints.get(instance_key, "") if sim_pins_hints else ""  # Read the forward-converter Sim.Pins role hint.
        hint_value = value_hints.get(instance_key, "") if value_hints else ""  # Read the forward-converter display value hint.
        candidate_ids = _candidate_lib_ids(element, model_types, allow_generic_subcircuits, hinted_lib_id)  # Derive the candidate library identifiers, including safe inverse-converter hints.
...
def _build_one_record(  # Build one component record from a resolved device and symbol.
    ...
    hinted_lib_id: str = "",  # Accept the forward-converter library identifier hint.
    ...
    hinted = bool(hinted_lib_id) and hinted_lib_id == lib_id  # Mark records resolved through the forward-converter hint.
    has_hint = bool(hinted_lib_id)  # Mark records that carry any forward-converter metadata hint.
    ...
    if prefix in {"X", "D", "J", "M", "Q", "Z"}:  # Split model-bearing device payloads into KiCad simulation fields.
        ...
        if hinted and prefix == "X" and not hint_value:  # Hinted subcircuits restore the authored library symbol.
            library_default_value = symbol_props.get("Value", "")  # Read the library symbol's default value.
            if library_default_value:  # Prefer the library default over the netlist subcircuit name.
                value = library_default_value  # Restore the ground-truth visible value.
        if hint_value and prefix == "X":  # The recorded display value is the authored truth even when the symbol fell back.
            value = hint_value  # Restore the exact visible value.
    ...
    record = {  # Assemble the ordinary component record.
        ...
        "instance_sim_library": "",  # Store the recovered library reference for hinted subcircuits.
        "instance_sim_pins": "",  # Store the synthesized pin-role mapping for hinted subcircuits.
        "extra_pins": [],  # Store no-connect symbol pins that the deck does not list.
        "hinted": hinted,  # Store the hint resolution flag for annotation preservation.
        "x": 0.0,  # Initialize the placement X.
        "y": 0.0,  # Initialize the placement Y.
        "angle": 0.0,  # Initialize the placement angle.
    }  # Finish the ordinary component record.
    if has_hint and prefix == "X":  # Restore the ground-truth simulation metadata for hinted subcircuits.
        record["instance_sim_library"] = sim_library  # Recover the library reference from the deck's include directives.
        if hint_roles:  # Prefer the authored role mapping recorded by the forward converter.
            role_values = [token.split("=", 1)[1].strip() if "=" in token else token for token in hint_roles.split()]  # Read the role names in the emitted node order.
            record["instance_sim_pins"] = " ".join(f"{position + 1}={role}" for position, role in enumerate(role_values))  # Re-key the roles onto the regenerated symbol's positional pins.
        else:  # Synthesize the identity mapping when the schematic recorded none.
            mapped_pins = sorted(pin_map.values(), key=_pin_sort_key)  # Order the mapped pins numerically.
            record["instance_sim_pins"] = " ".join(f"{pin_number}={pin_number}" for pin_number in mapped_pins)  # Synthesize the identity role mapping used by the forward converter.
    if hinted and prefix == "X":  # Declare unlisted no-connect pins on the resolved library symbol.
        mapped_pins = sorted(pin_map.values(), key=_pin_sort_key)  # Order the mapped pins numerically.
        no_connect_numbers = _no_connect_pin_numbers(symbol_node)  # Read the symbol's no-connect ports.
        record["extra_pins"] = sorted((no_connect_numbers - set(mapped_pins)), key=_pin_sort_key)  # Declare unlisted no-connect pins on the instance.
    return True, record, "", 0  # Return the built record.
...
def _route_and_build(root_uuid: str, records: List[Dict[str, Any]], settings: Dict[str, Any], forced_strategy: Optional[str] = None) -> BuildResult:  # Place symbols with hybrid optimization, route nets with grid A*, and assemble schematic nodes.
    layout = _layout_parameters(settings, len(records))  # Resolve the layout parameters from the settings.
...
def _straighten_aligned_nets(  # Replace collinear multi-hop nets with single straight rails.
    ...
    for node_name, rail in rail_points.items():  # Walk the candidate rails.
        ...
        for point in foreign_points:  # Check every foreign point against the rail interior.
            ...
            if horizontal and abs(point[1] - low_y) < _KI_CAD_CONTACT_TOLERANCE and low_x < point[0] < high_x:  # Detect a foreign point inside the horizontal span.
                clear = False  # Reject the rail.
                break  # Stop checking.
            if not horizontal and abs(point[0] - low_x) < _KI_CAD_CONTACT_TOLERANCE and low_y < point[1] < high_y:  # Detect a foreign point inside the vertical span.
                clear = False  # Reject the rail.
                break  # Stop checking.
...
def _layout_parameters(settings: Dict[str, Any], symbol_count: int) -> Tuple[float, int, float, float]:  # Resolve the validated layout parameters.
    ...
    _paper_name, page_width, page_height = _page_size_for_symbol_count(settings, symbol_count)  # Pick the page for the design size.
    page_width = float(settings.get("kicad_sch_page_width", page_width))  # Honor an explicit page width override.
    page_height = float(settings.get("kicad_sch_page_height", page_height))  # Honor an explicit page height override.
...
def _write_generated_subcircuit_symbol_file(  # Dynamically generate a validated KiCad symbol library file on the fly.
    subcircuit_name: str,  # Accept the netlist subcircuit name.
    node_count: int,  # Accept the netlist node count.
    port_names: Sequence[str],  # Accept the resolved pin names.
    temp_directory: str,  # Accept the scratch directory for the generated file.
    settings: Dict[str, Any],  # Accept the normalized settings.
) -> BuildResult:  # Return the generated lib_id and parsed symbol node.
    build_result = _build_generic_subcircuit_symbol(subcircuit_name, node_count, port_names)  # Build the self-contained symbol definition.
    if not build_result[0]:  # Stop when the symbol build fails.
        return build_result  # Return the build error unchanged.
    lib_id, symbol_node = build_result[1]  # Read the generated lib_id and symbol node.
    symbol_name = _split_lib_id(lib_id)[1]  # Read the generated symbol stem.
    version = settings.get("kicad_symbol_version") or datetime.date.today().strftime("%Y%m%d")  # Resolve the library format version.
    generator = settings.get("kicad_symbol_generator") or "electronics_design"  # Resolve the generator name.
    root = SExp(name="kicad_symbol_lib", children=[  # Build the symbol library root.
        SExp(name="version", children=[SExp(value=version)]),  # Library format version.
        SExp(name="generator", children=[SExp(value=generator)]),  # Generator identifier.
        symbol_node,  # The generated symbol definition.
    ])  # Finish the library root.
    output_path = os.path.join(temp_directory, symbol_name + _KICAD_SYMBOL_EXTENSION)  # Build the scratch symbol file path.
    write_result = _write_text_file(output_path, "\n".join(_serialize_sexp(root)) + "\n")  # Write the generated library file.
    if not write_result[0]:  # Stop when the file cannot be written.
        return False, None, "WRITE_ERROR", 0  # Return the write error.
    validation = is_valid_kicad_symbol_file(output_path)  # Validate the freshly written symbol library.
    if not validation[0]:  # Stop when the generated symbol fails validation.
        return False, None, f"INVALID_GENERATED_KICAD_SYMBOL: {validation[1]}", 0  # Return the validation error.
    try:  # Attempt to read the generated symbol file.
        with open(output_path, "r", encoding="utf-8") as file_handle:  # Open the generated symbol text.
            fallback_root = parse_string(file_handle.read())  # Parse the generated symbol library.
    except OSError:  # Treat unreadable generated files as a write failure.
        return False, None, "WRITE_ERROR", 0  # Return the write error.
    for candidate_node in fallback_root.find_children("symbol"):  # Walk the top-level symbol definitions.
        candidate_values = [child.value for child in candidate_node.children if child.is_atom]  # Collect the name atoms.
...
def _classify_generic_port(port_name: str) -> Tuple[str, str]:  # Classify one subcircuit port into a KiCad pin type and symbol side.
    upper = port_name.upper()  # Normalize the port name for matching.
    if port_name in _GROUND_NODE_NAMES:  # Ground-named ports become bottom power pins.
        return "power_in", "bottom"  # Return the ground pin role.
    if "+" in upper or upper in {"VCC", "VDD", "VEE", "VSS", "VBAT"} or upper.startswith("N+"):  # Power-named ports become top power pins.
        return "power_in", "top"  # Return the supply pin role.
    if "OUT" in upper:  # Output-named ports become right-side outputs.
        return "output", "right"  # Return the output pin role.
    return "input", "left"  # Everything else is an input on the left side.


def _spread_generic_pins(count: int) -> List[float]:  # Spread pins on one symbol side symmetrically around the origin.
    if count <= 0:  # Skip empty sides.
        return []  # Return no positions.
    return [round((index - (count - 1) / 2.0) * 2.54, 4) for index in range(count)]  # Stack at a 2.54 mm pitch.


def _generic_pin_node(pin_number: int, pin_type: str, pin_x: float, pin_y: float, pin_angle: float, pin_name: str, pin_length: float = 2.54) -> SExp:  # Build one generated pin node.
    effects = SExp(name="effects", children=[SExp(name="font", children=[SExp(name="size", children=[SExp(value=0.635), SExp(value=0.635)])])])  # Small pin-name font.
    number_effects = SExp(name="effects", children=[SExp(name="font", children=[SExp(name="size", children=[SExp(value=1.27), SExp(value=1.27)])])])  # Standard pin-number font.
    return SExp(name="pin", children=[  # Build the pin node.
        SExp(value=pin_type),  # Electrical type.
        SExp(value="line"),  # Line graphical style.
        SExp(name="at", children=[SExp(value=pin_x), SExp(value=round(pin_y, 4)), SExp(value=pin_angle)]),  # Connection point.
        SExp(name="length", children=[SExp(value=pin_length)]),  # Pin length.
        SExp(name="name", children=[SExp(value=pin_name, _originally_quoted=True), effects]),  # Pin name.
        SExp(name="number", children=[SExp(value=str(pin_number), _originally_quoted=True), number_effects]),  # Pin number.
    ])  # Return the assembled pin node.


def _generic_stroke_node(width: float) -> SExp:  # Build one stroke section for generated symbol graphics.
    return SExp(name="stroke", children=[  # Build the stroke section.
        SExp(name="width", children=[SExp(value=width)]),  # Stroke width.
        SExp(name="type", children=[SExp(value="default")]),  # Default stroke type.
    ])  # Return the stroke node.


def _generic_fill_node(fill_type: str) -> SExp:  # Build one fill section for generated symbol graphics.
    return SExp(name="fill", children=[SExp(name="type", children=[SExp(value=fill_type)])])  # Return the fill node.


def _generic_polyline_node(points: Sequence[Tuple[float, float]], width: float, fill_type: str) -> SExp:  # Build one polyline for generated symbol graphics.
    return SExp(name="polyline", children=[  # Build the polyline node.
        SExp(name="pts", children=[SExp(name="xy", children=[SExp(value=x), SExp(value=y)]) for x, y in points]),  # Point list.
        _generic_stroke_node(width),  # Stroke section.
        _generic_fill_node(fill_type),  # Fill section.
    ])  # Return the polyline node.


def _generic_circle_node(center_x: float, center_y: float, radius: float, width: float, fill_type: str) -> SExp:  # Build one circle for generated symbol graphics.
    return SExp(name="circle", children=[  # Build the circle node.
        SExp(name="center", children=[SExp(value=center_x), SExp(value=center_y)]),  # Circle center.
        SExp(name="radius", children=[SExp(value=radius)]),  # Circle radius.
        _generic_stroke_node(width),  # Stroke section.
        _generic_fill_node(fill_type),  # Fill section.
    ])  # Return the circle node.


def _generic_text_node(text: str, x: float, y: float, size: float) -> SExp:  # Build one text node for generated symbol graphics.
    return SExp(name="text", children=[  # Build the text node.
        SExp(value=text, _originally_quoted=True),  # Text value.
        SExp(name="at", children=[SExp(value=x), SExp(value=y), SExp(value=0)]),  # Text position.
        SExp(name="effects", children=[SExp(name="font", children=[SExp(name="size", children=[SExp(value=size), SExp(value=size)])])]),  # Font effects.
    ])  # Return the text node.


def _build_generic_subcircuit_symbol(subcircuit_name: str, node_count: int, port_names: Optional[Sequence[str]] = None) -> Tuple[bool, Tuple[str, SExp]]:  # Build a self-contained generic symbol for an unresolvable subcircuit.
    ...
    common_symbol_name = f"{symbol_name}_0_1"  # Build the shared body-graphics sub-symbol name.
    ...
    resolved_names = [port_names[index] if port_names and index < len(port_names) else f"P{index + 1}" for index in range(node_count)]  # Resolve every pin name with a numeric fallback.
    left_pins: List[Tuple[int, str]] = []  # Collect left-side input pins.
    right_pins: List[Tuple[int, str]] = []  # Collect right-side output pins.
    top_pins: List[Tuple[int, str]] = []  # Collect top-side power pins.
    bottom_pins: List[Tuple[int, str]] = []  # Collect bottom-side power pins.
    for pin_number, resolved_name in enumerate(resolved_names, start=1):  # Classify every port by its name.
        pin_type, side = _classify_generic_port(resolved_name)  # Resolve the pin role.
        if side == "right":  # Route output ports to the right side.
            right_pins.append((pin_number, resolved_name))  # Record the output pin.
        elif side == "top":  # Route supply ports to the top side.
            top_pins.append((pin_number, resolved_name))  # Record the top power pin.
        elif side == "bottom":  # Route ground ports to the bottom side.
            bottom_pins.append((pin_number, resolved_name))  # Record the bottom power pin.
        else:  # Route everything else to the left side.
            left_pins.append((pin_number, resolved_name))  # Record the input pin.

    half_height = max(5.08, max((abs(y) for y in _spread_generic_pins(max(len(left_pins), len(right_pins)))), default=0.0) + 1.27)  # Grow the triangle to cover every side pin.
    power_y = half_height + 2.54  # Place power pins beyond the side-pin spread so rotation never merges their column with side pins.
    power_length = 0.25 * half_height + 2.54  # Keep every power-pin body touching the triangle edge at the scaled height.
    pin_nodes: List[SExp] = []  # Collect the generated pin nodes.
    for (pin_number, resolved_name), pin_y in zip(left_pins, _spread_generic_pins(len(left_pins))):  # Place left-side inputs.
        pin_nodes.append(_generic_pin_node(pin_number, "input", -7.62, pin_y, 0, resolved_name))  # Append the left input pin.
    for (pin_number, resolved_name), pin_y in zip(right_pins, _spread_generic_pins(len(right_pins))):  # Place right-side outputs.
        pin_nodes.append(_generic_pin_node(pin_number, "output", 7.62, pin_y, 180, resolved_name))  # Append the right output pin.
    for top_index, (pin_number, resolved_name) in enumerate(top_pins):  # Place top-side power pins.
        pin_nodes.append(_generic_pin_node(pin_number, "power_in", -2.54 + top_index * 2.54, power_y, 270, resolved_name, power_length))  # Append the top power pin.
    for bottom_index, (pin_number, resolved_name) in enumerate(bottom_pins):  # Place bottom-side power pins.
        pin_nodes.append(_generic_pin_node(pin_number, "power_in", -2.54 + bottom_index * 2.54, -power_y, 90, resolved_name, power_length))  # Append the bottom power pin.

    common_graphics: List[SExp] = [  # Start the shared body graphics.
        _generic_polyline_node([(-5.08, half_height), (5.08, 0), (-5.08, -half_height), (-5.08, half_height)], 0.254, "background"),  # Op-amp triangle body.
    ]  # Finish the base body graphics.
    if any("REF" in resolved_name.upper() for resolved_name in resolved_names):  # Draw a reference-voltage block when the subcircuit has a REF port.
        left_ys = _spread_generic_pins(len(left_pins))  # Read the left pin rows.
        ref_y = max((min(left_ys) if left_ys else -2.54) - 2.54, -6.35)  # Place the REF circle below the lowest left pin inside the pin envelope.
        ref_center_x = -6.35  # Keep the circle inside the left-pin column so the routing body never covers a pin cell.
        common_graphics.extend([  # Append the REF block graphics.
            _generic_circle_node(ref_center_x, ref_y, 1.27, 0, "background"),  # REF circle.
            _generic_polyline_node([(ref_center_x, ref_y + 2.54), (ref_center_x, ref_y + 1.27)], 0, "none"),  # Lead from the lowest input row to the circle.
            _generic_text_node("REF", ref_center_x, ref_y, 0.635),  # REF text label.
        ])  # Finish the REF block.
        if bottom_pins and ref_y - 1.905 >= -power_y:  # Connect the REF circle toward the bottom power pin when the lead can land on its body.
            common_graphics.append(_generic_polyline_node([(ref_center_x, ref_y - 1.27), (ref_center_x, ref_y - 1.905), (-2.54, ref_y - 1.905)], 0, "none"))  # Lead from the circle bottom to the power pin.
        elif not bottom_pins:  # Draw a short stub below the REF circle when no power pin exists.
            common_graphics.append(_generic_polyline_node([(ref_center_x, ref_y - 1.27), (ref_center_x, ref_y - 2.54)], 0, "none"))  # Stub lead.
    ...
    symbol_children: List[SExp] = [  # Start the generic symbol children.
        ...
        SExp(name="symbol", children=[SExp(value=common_symbol_name)] + common_graphics),  # The shared body-graphics sub-symbol.
...
def _build_symbol_instance_nodes(records: Sequence[Dict[str, Any]], root_uuid: str) -> List[SExp]:  # Build KiCad symbol instance nodes for every record.
    ...
    for record_index, record in enumerate(records):  # Walk every component record with a fallback identity for generated ground symbols.
        ...
        pin_numbers.extend(record.get("extra_pins", []))  # Append unlisted no-connect pins so the instance declares every symbol pin.
        for pin_number in pin_numbers:  # Walk the declared pins.
...
def _build_instance_properties(record: Dict[str, Any]) -> List[SExp]:  # Build the property nodes for one symbol instance.
    ...
    if record.get("instance_sim_library"):  # Restore the recovered model library as a hidden simulation property.
        properties.append(_property_node(record["reference"], "Sim.Library", str(record["instance_sim_library"]), record, visible=False))
    if record.get("instance_sim_pins"):  # Restore the synthesized pin-role mapping as a hidden simulation property.
        properties.append(_property_node(record["reference"], "Sim.Pins", str(record["instance_sim_pins"]), record, visible=False))
...
def _assemble_schematic(  # Assemble the final schematic text from its parts.
    ...
    for lib_id, symbol_node in all_embedded.items():  # Walk the embedded symbol definitions.
        renamed_node = _rename_embedded_symbol(symbol_node, lib_id, preserve_annotations=lib_id in hinted_set)  # Qualify the symbol name with its library identifier.
...
def _rename_embedded_symbol(symbol_node: SExp, lib_id: str, preserve_annotations: bool = False) -> SExp:  # Rename an embedded symbol definition to its full library identifier.
    if preserve_annotations:  # Hinted subcircuit symbols keep their authored annotation visibility.
        ...
        for child in symbol_node.children:  # Walk the symbol children.
            ...
            else:  # Keep every remaining child byte-for-byte equivalent.
