...
4. **Place** — `kicad_pcb_placement_strategy` selects `schematic` (default; scales the schematic signal-flow layout onto the board) or `rows` (deterministic row packing); overlapping bodies are pushed apart, positions snap to a 0.1 mm grid, and the outline grows (or the drawing scales) to fit.  With `kicad_pcb_compact_placement` enabled (default) the legalized layout is then compacted: a deterministic 1-D first-fit sweep on both axes pulls every footprint toward the origin until only `kicad_pcb_component_spacing` mm separates neighbouring courtyards, the tightest of two orderings wins, and the board outline shrinks to the compacted content plus `kicad_pcb_margin`.  A final check rejects any placement that breaks the minimum gap.
...
6. **Route** — every ordinary net is autorouted by the kicad-tools grid A* router with the configured track width, clearance, grid resolution, and via sizes; routed segments and vias are written back through the PCB model.  After routing, every routed chain is rebuilt so wire bends honour `kicad_pcb_min_wire_angle`: sharp corners are replaced by tangent fillets that keep the endpoints, layer transitions, and pad/via connectivity untouched, and each fillet is clearance-checked against the other nets before it is accepted.  Because the threshold is a free value in `(0, 180)`, bends of 120°, 135°, 150°, and anything in between can be requested; values above the router's 45° grid are approximated by shorter tangent chords (best effort near the serialization precision).  Because the generated boards carry no copper pours, power nets such as `GND` are routed as ordinary signals; pass `kicad_pcb_skip_route_nets` to exclude plane nets instead.
...
convert_settings.update({
    ...
    "kicad_pcb_compact_placement": True,  # Shrink the placed layout to the densest legal packing.
    "kicad_pcb_component_spacing": 0.5,  # Minimum courtyard-to-courtyard gap in mm (>= 0).
    "kicad_pcb_min_wire_angle": 120.0,   # Minimum angle between two wires at a bend, degrees (0, 180).
    "kicad_pcb_wire_bend_chamfer": 0.5,  # Corner cut length used to smooth a sharp bend in mm.
...
1. **netlist → symbol_initial** — generates JSON with `SYMBOL`, `X=0`, `Y=0`, `ORIENTATION=""`, empty `RECTANGLE` and `PINS`. Every `X` device is validated against the resolved `.asy` `SpiceOrder` values: a symbol pin outside the `X` line or a connected deck node left uncovered by any `SpiceOrder` returns `X_PIN_COUNT_MISMATCH` with the failing line number. Set `ltspice_allow_spice_order_mismatch` to `True` to restore the historical silent skip.
...
convert_settings = {
    ...
    "ltspice_allow_spice_order_mismatch": False,  # Set True to skip strict X-line SpiceOrder validation.
