...
| `UNSUPPORTED_DEVICE` | A netlist device has no KiCad schematic representation (for example `K`, `A`, `@`, or `&` prefixes), or its shape cannot round-trip through `kicad_sch_to_ltspice_netlist` (for example a voltage source with a floating negative node represented by a power symbol). | Rework the netlist to use supported device shapes, or resolve the device to an `.asy`/library symbol that the reverse converter can emit. |
