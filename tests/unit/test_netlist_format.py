"""Unit tests for LTspice netlist format validation."""  # Describe the unit-test module purpose.
...
_VALID_DIRECTORY = _ROOT_DIRECTORY / "test_files" / "netlist_format" / "valid"  # Point to valid format fixtures.
_INVALID_DIRECTORY = _ROOT_DIRECTORY / "test_files" / "netlist_format" / "invalid"  # Point to invalid format fixtures.


class TestNetlistFormat(unittest.TestCase):  # Group format-validator test cases together.
    def test_valid_format_fixtures(self) -> None:  # Verify that all valid fixtures pass the public validator.
        for fixture_path in sorted(_VALID_DIRECTORY.glob("*.net")):  # Walk every valid format fixture file.
            result = is_valid_ltspice_netlist_format(str(fixture_path))  # Execute the public validator on the fixture path.
    ...
    def test_invalid_format_fixtures(self) -> None:  # Verify that all invalid fixtures fail the public validator.
        for fixture_path in sorted(_INVALID_DIRECTORY.glob("*.net")):  # Walk every invalid format fixture file.
            ...
            self.assertIn("Line format/spacing is invalid!", result[1], msg=f"{fixture_path.name} should report a format error.")  # Assert that the correct error prefix is returned.
