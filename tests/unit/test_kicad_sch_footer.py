"""Unit tests for KiCad schematic footer validation."""  # Describe the unit-test module purpose.
...
_VALID_DIRECTORY = _ROOT_DIRECTORY / "test_files" / "kicad_sch_footer" / "valid"  # Point to valid schematic footer fixtures.
_INVALID_DIRECTORY = _ROOT_DIRECTORY / "test_files" / "kicad_sch_footer" / "invalid"  # Point to invalid schematic footer fixtures.


class TestKicadSchFooter(unittest.TestCase):  # Group KiCad schematic footer-validator test cases together.
    def test_valid_footer_fixtures(self) -> None:  # Verify that all valid schematic footer fixtures pass the public validator.
        for fixture_path in sorted(_VALID_DIRECTORY.glob("*.kicad_sch")):  # Walk every valid schematic footer fixture file.
            result = is_valid_kicad_sch_footer(str(fixture_path))  # Execute the public validator on the fixture path.
    ...
    def test_invalid_footer_fixtures(self) -> None:  # Verify that all invalid schematic footer fixtures fail the public validator.
        for fixture_path in sorted(_INVALID_DIRECTORY.glob("*.kicad_sch")):  # Walk every invalid schematic footer fixture file.
